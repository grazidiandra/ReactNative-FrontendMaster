...
const AddNewPaletteModal = ({ navigation }) => {
  ...
  const handlesubmit = useCallback(() => {
      if(!name) {
        ...
        navigation.navigate('Home', {
          newPalette: { paletteName: name, colors: selectedColor },
        });
      }
    },
    [name, selectedColor],
  ...
  const handleUpdate = useCallback((newValue, color) => {
      if (newValue === true) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.name}>Name of your new color pallete</Text>
      <TextInput value={name} onChangeText={value => setName(value)} style={styles.inputName}/>
        ...
        renderItem={({ item }) => (
          <View style={styles.color}>
              ...
              onValueChange={newValue => handleUpdate(newValue, item)}/>
          </View>
        )}
        />
